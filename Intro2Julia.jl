...
include("/home/albert/Desktop/teaching/src/StructuredGrid.jl");
...
We can write it in almost as in MATLAB:
"""
...
5. Micro-optimization. E.g.:
    ...
    * Multiplication is ~$$\times 5$$ times faster than division
...
# ╔═╡ 9e896084-2a28-4c34-a2d3-9ee6968dd204
# gr.x
...
md"""
## Can we do better?
```gr``` is an object that contains 3 arrays $x$, $y$ and $z$ that are allocated when ```gr``` is instantiated. We actually do not need to allocate any array, and create a lazy object instead if we know one corner of the grid and the distance between consecuative nodes.
"""
